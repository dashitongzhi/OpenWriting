...
- 默认遵循用户/系统层级的 GitHub Delivery：除非用户明确要求 review-only、不修改、不提交、不 push，仓库实现任务完成验证后应优先创建任务分支、提交、push，并打开 PR。
- 不要直接 push 到 `main`，除非用户明确要求；使用短描述任务分支并只提交本任务相关文件。
- 工作区可能有用户改动；不要 reset、checkout 或 revert 未确认的改动，也不要把无关改动混入提交或 PR。
- 如果 GitHub 权限、认证、CI 或分支保护阻止 push/PR，汇报清楚 blocker，并保留本地已验证的分支或提交。
